...
public class HermesRuntimeHintsTests {

	@Test
	void registerHints() {
		RuntimeHints runtimeHints = new RuntimeHints();
		HermesRuntimeHints hermesRuntimeHints = new HermesRuntimeHints();
		hermesRuntimeHints.registerHints(runtimeHints, null);

		Set<TypeReference> jsonAnnotatedClasses = findJsonAnnotatedClassesInPackage("io.github.partmeai.hermes");
		...
		assertThat(registeredTypes.contains(TypeReference.of(HermesApi.ChatRequest.class))).isTrue();
		assertThat(registeredTypes.contains(TypeReference.of(HermesApi.ChatRequest.Tool.class))).isTrue();
		assertThat(registeredTypes.contains(TypeReference.of(HermesApi.Message.class))).isTrue();
		assertThat(registeredTypes.contains(TypeReference.of(HermesChatOptions.class))).isTrue();
	}
	...
	void registerHintsWithNullClassLoader() {
		...
		org.assertj.core.api.Assertions.assertThatCode(() -> hermesRuntimeHints.registerHints(runtimeHints, null))
	...
	void verifyJsonAnnotatedClassesFromCorrectPackage() {
		...
		for (TypeReference classRef : jsonAnnotatedClasses) {
			assertThat(classRef.getName()).startsWith("io.github.partmeai.hermes");
		}
	}
	...
	void verifyHintsRegistrationWithCustomClassLoader() {
		...
		org.assertj.core.api.Assertions
			.assertThatCode(() -> hermesRuntimeHints.registerHints(runtimeHints, customClassLoader))
			.doesNotThrowAnyException();
	...
	void verifyNoProxyHintsAreRegistered() {
		...
		// Hermes should only register reflection hints, not proxy hints
	...
	void verifyNoSerializationHintsAreRegistered() {
		...
		// Hermes should only register reflection hints, not serialization hints
